...
/**
 * auth	：fire
 * content：根据外部分类和款式编号查找速易宝一级和二级分类编号
 * time	：2017-10-31
 **/
function getCategoryMap($categoryList,$out_cid,$out_jid)
{
    $res_cid=0;
    $res_jid=0;
    foreach ($categoryList as $v){
        if($v['out_code']==$out_jid){
            $res_jid=$v['id'];
            foreach ($categoryList as $vv){
                if($vv['out_code']==$out_cid&&$v['pid']==$vv['id']){
                    $res_cid=$vv['id'];
                    break;
                }
            }
    ...
    return ["cid"=>$res_cid,"jid"=>$res_jid];
}
 ...
 * @param  method 加密算法,base64_encode,sha1,crypt,urlencode
 * @return string 返回参数签名值
 */
function getSignature($params, $secret,$method=null)
{
    $str = '';  //待签名字符串
    //先将参数以其参数名的字典序升序进行排序
    ksort($params);
...
/**
 * auth	：fire
 * content：根据token检查用户编号
 * time	：2017-10-31
 **/
function get_userid_token(){
    $agent_id=get_agent_id();
    $tokenstr	=get_header_token();
    if($agent_id && $tokenstr && $tokenstr != 'null'){
        vendor('jwt.JWT');
        $token= \JWT::decode($tokenstr, config("login_key"), array('HS256'));
        if($token->exp_time>time()){ 
            return $token->uid;
        }
    }
    return null;
 ...
 * @return string
 */
function pwdHash($password,$name,$agent_id){
    return md5($password .$name.$agent_id.md5(config('salt')));
}
 ...
 * @return int
 */
function get_agent_client($type=0){
    $agent_id=get_agent_id();
    $agent_clients=db('agent_clients')
    ->where("agent_id",$agent_id)
    ->cache("clients_".$agent_id,300)
    ->field("type,key,secret")
    ->select();
    if($agent_clients){
        foreach ($agent_clients as $v){
            if($v['type']==$type){ 
                return $v;break;
            }
        }
...
function get_agent_id(){
    $http_host=get_agent_host();
    $agent_id=cache("agent_id_".$http_host);    
    if(empty($agent_id)){
        $agent=get_agent_base($http_host);
        if($agent){
            $agent_id=$agent['id'];
            cache("agent_id_".$http_host,$agent_id);
        }else{
            //由于nginx做了通配
 ...
 * @return int
 */
function get_agent_base($http_host=null){ 
    if(empty($http_host)){
        $http_host=get_agent_host();
    }        
    $agent=db('agent')
        ->where("domain|mobile_domain",$http_host)
        ->cache("agent_".$http_host,5)
        ->find();     
    return $agent;
}
 ...
 * @return int
 */
function get_agent_info($agent_id=null){ 
    if(empty($agent_id)){
		$agent_id=get_agent_id();
    }        
    $agent=db('agent_info')
 ...
 * @return int
 */
function get_agent_trader($agent_id=null){ 
    if(empty($agent_id)){
        $agent_id=get_agent_id();
    }        
    $trader=db('trader')
 ...
 * @return int
 */
function get_agent_wxuser_base(){
    $agent_id=get_agent_id();
    $wxuser=db('wx_user')
        ->where("agent_id",$agent_id)
        ->cache("wxuserbase_".$agent_id,5)
        ->field("appid,appsecret,aeskey,w_token")
        ->find();
    return $wxuser;
}
 ...
 * @return int
 */
function get_agent_wxuser_info(){
    $agent_id=get_agent_id();
    $wxuser=db('wx_user')
        ->where("agent_id",$agent_id) 
        ->find();
 ...
 * @return array|bool
 */
function array2treesingle(&$array,$pid=0,$pid_name='pid',$id_name='id'){
    $result=array();    
    foreach ($array as $v){
        if($v[$pid_name]==$pid){
            $v['sub']=array2treesingle($array,$v[$id_name]);           
            $result[]=$v;            
        }
    }
    return $result;
 ...
 * @return array|bool
 */
function array2tree(&$array, $pid_name = 'pid', $child_key_name = 'children',$id_name='id')
{
    ...
    while (isset($counter[0]) && $counter[0] > 0) {
        ...
        if (isset($counter[$temp[$id_name]]) && $counter[$temp[$id_name]] > 0) {
            ...
            if ($temp[$pid_name] == 0) {
                ...
                $array = array_child_append($array, $temp[$pid_name], $temp, $child_key_name,$id_name);
            }
        }
        $counter = array_children_count($array, $pid_name);
    }
 ...
 * @return mixed
 */
function array_child_append($parent, $pid, $child, $child_key_name,$id_name)
{
...
/**
 * 简单curl访问
 * @param $postString提交字段
 * @param $httpHeader 头部数组
 * @return string
 */
function httpSigle($url,$postString='',$httpHeader='')
{
    $ch = curl_init();
 ...
 * @return 
 */
function ConditionShow($a='',$b,$c=''){
    $m = new app\common\logic\ConditionShow();
    return $m->ConditionShowDefault($a,$b,$c);
}

//验证码
function getrandcode($length,$chars){
	$code = '';
	$max = strlen($chars) - 1;
	for($i = 0; $i < $length; $i++) {
...
function logic($Logicname){
	$Logicname = "\\app\common\logic\\".$Logicname;
	return new $Logicname();
}

/**
 * 根据code和sort过滤广告项
 * $items 根据page查找出来的广告项列表
 * $code 广告项命名
 * $sort 广告项排序
 */
function tfilter($items,$code,$sort=0){
    $result=[];
    $k=0;
    if($items){
        foreach ($items as $v){
...
{
	$client_id = config('ZMFXWEB.client_id');;
	$client_secret = config('ZMFXWEB.client_secret');;
    $commonParams = [
        'client_id' => $client_id,
    ...
    foreach($params as $k => $param){
        if(in_array($k, ['client_id', 'sign'])){
           continue;
        }
        $extraParams[$k] = $param;
    }

    $arr = array_merge($commonParams, $extraParams);
...
function updateGiaData($zid)
{
    ...
    try {
        ...
        $document=httpSigle("https://www.gia.edu/otmm_wcs_int/loadXML.jsp?ReportNumber=$matchs[1]");
        $result   = (array)(simplexml_load_string(autoToutf8($document))->REPORT_DTLS->REPORT_DTL);
    
        $data['zid']=$zid;
        $data['ztype']=1;
        $data['data']=json_encode($result);
        $data['pdf']='https://www.gia.edu/otmm_wcs_int/proxy-pdf/?ReportNumber='.$zid.'&url=https://myapps.gia.edu/RptChkClient/reportClient.do?ReportNumber='.$matchs[1];
        Db::name('report_certificate2')->insert($data);
        Db::name('goods_diamond')->where("certificate_number",$zid)->update(['certificate_isself' => 1]);
        array_push($successno, $zid);        
        return $data;    
    } catch (\Exception $e) {
        Db::name('goods_diamond')->where("certificate_number",$zid)->update(['certificate_isself' => 2]);
        array_push($failno, $zid);
    }
}
