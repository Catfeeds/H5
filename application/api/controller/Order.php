...
class Order extends ApiBase
{
     ...
     * 
     */
    public function getpayment($total_price){
	   	$this->checkUserLogin();
        $res = get_agent_payment($total_price); 
    ...
    public function addpre(){
        $uid=$this->checkUserLogin();
        $data['user_id'] = $uid; //一定要加数组 sxm
        D('cart')->refreshCart($data); //更新购物车数据 
    ...
    public function addconfirm($id){
        ...
        if(!$order){
            return json_encode(['status'=>1011,'msg'=>'订单不存在']);
        }
        ...
        if($order['pay_status']==0&&$order['pay_type']==2&&$order['add_time']<date('Y-m-d H:i:s',time()-900)){
            return json_encode(['status'=>1012,'msg'=>'该订单已过期']);
        }
        ...
        if($order['pay_status']==0){ //未支付
            if($order['pay_type']==2){ //在线支付 未支付 跳转支付页面
                if($payment[$order['pay_code']]['url']){
                    ...
                    return json_encode(['status'=>1013,'msg'=>'支付方式未配置']);           
                }
            }
        }
     ...
     * 		支付方式		 `pay_type` => '1 线下转账 2 在线支付',
  						 `pay_code` => '支付code',
  						 `pay_name` => '支付方式名称',
     * 		配送方式		 `shipping_type` => '1 上门自取 2 第三方物流',
  						 `shipping_code` => '物流code',
  						 `shipping_name` => '第三方物流 物流名称 ，上门自取 门店名称',
     * 		用户留言备注 	 `user_note` => COMMENT '用户备注',
     * ]
     */
    public function updateinfo() {
    	...
    	//接收参数
        $post = I('post.');
        $order_info = model('Order')->getInfoBySn($post['order_sn']);
        if (!$order_info) {
        	return json(['status'=>101,'msg'=>'订单不存在']);
        } else {
        	$result = model('Order')->updateInfo($post);
        	if ($result !== FALSE) {
        		return json(['status'=>100,'msg'=>'更新成功','data'=>$result]);
        	} else {
        		return json(['status'=>103,'msg'=>'更新失败','data'=>'']);
        	}
        }
    }
}