	...
	var BringAjax = {
			IntendData	 : {													
							...
							EasyClickSeat	: ' .EasyClick',				//默认点击事件发起点
							ClickFuncSeat	: ' .ClickFunc',				//默认点击事件发起点
			...
			Send		 : 	function(){
								...
								$[this.IntendData.Operation](this.IntendData.Url, this.Interactive[0],function(data) {
									if(data){
										...
										if(_this.CacheData[_this.IntendData.CacheHtmlKey].status==100){
											if(_this.IntendData.SwitchMain){
												(_this.Interactive[1] == undefined)			?	CommonFunction.Prompt.Tan(BringAjax.CacheData[BringAjax.IntendData.CacheHtmlKey].msg) : _this.Interactive[1]();
	...
	var EasyClick = function() {
		$(BringAjax.IntendData.EasyClickSeat).click(function(){
			BringAjax.Send();
		});
	}	
	
	//简易点击事件发起
	var ClickFunc = function(Springboard) {
		$(BringAjax.IntendData.ClickFuncSeat).click(Springboard);
	}	
	...
	var CommonFunction ={
		...
		Prompt :{																						//提示
			WapTan		: function(msg,needsec){
				layer.open({
				   content: msg,
				   time: 2
				});
			},
			PcTan		: function(msg,needsec){
				layer.msg(msg,{
					shift: needsec ? needsec : 6,
				});
			},
			Tan  		: function(msg,needsec){
				if(window.conf.Client==1){
					this.PcTan(msg,needsec);
				}else{
					this.WapTan(msg,needsec);
				}
				return false;
			},			
		},
